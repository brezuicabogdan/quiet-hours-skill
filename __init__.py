...
class QuietHours(MycroftSkill):
    ...
    def _init(self):
        ...
        """ Turn off quiet hours if previously on and settings change needs it to be off"""
        if(self.settings.get('enabled')):
    ...
    def on(self):
        if(not (self.settings.get('enabled') and not self.settings.get('active'))):
            return
        self.saved_volume = self.mixer.getvolume()
    ...
    def off(self):
        if(not (self.settings.get('enabled') and self.settings.get('active'))):
            return
        self.mixer.setvolume(self.saved_volume[0])
